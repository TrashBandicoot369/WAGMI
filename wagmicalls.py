...
SOURCE_GROUPS = [-1002416386782, -1002017173747]  # Group to monitor
    ...
    def store_token_data(self, token_data: Dict) -> bool:
        ...
        try:
            ...
            if existing_docs:
                ...
                ath_mc = doc_data.get('athMarketCap', token_data['marketCap'])  # fallback to current

                if initial_mc:
                    token_data['initialMarketCap'] = initial_mc

                    if token_data['marketCap']:
                        # Calculate % gain from initial to current
                        token_data['currentGainPercent'] = ((token_data['marketCap'] - initial_mc) / initial_mc) * 100

                        # Update ATH if this is higher
                        if token_data['marketCap'] > ath_mc:
                            ath_mc = token_data['marketCap']

                        token_data['athMarketCap'] = ath_mc
                        token_data['athGainPercent'] = ((ath_mc - initial_mc) / initial_mc) * 100
                ...
                logger.info(f"   - ATH MC: ${token_data.get('athMarketCap', 0):,.2f}")
                logger.info(f"   - ATH Gain: {token_data.get('athGainPercent', 0):,.2f}%")
                ...
                doc.reference.update({
                    ...
                    "athMarketCap": token_data["athMarketCap"],
                    "athGainPercent": token_data["athGainPercent"],
                    "totalGainPercent": token_data.get("currentGainPercent")
                })
                ...
                token_data['athMarketCap'] = token_data['marketCap']
                token_data['athGainPercent'] = 0
